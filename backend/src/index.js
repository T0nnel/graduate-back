...
app.post('/api/updateProfile', verifyToken, upload.single('profilePicture'), async (req, res) => {
    try {
        ...
        if (password) {
            user.password = await bcryptjs_1.default.hash(password, 10); // Hash the new password
        }
        // Handle file upload
        if (req.file) {
            if (user.profilePicture) {
                fs_1.default.unlinkSync(path_1.default.join(__dirname, 'uploads', user.profilePicture));
            }
